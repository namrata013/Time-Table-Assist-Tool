...
def searchCourse(root, entries, conn, lab):
    course = str(entries['Search'].get())
    course = course.upper()
    course = course.strip()
    if conn is not None:
        cur = conn.cursor()
    cur.execute("SELECT Course_No FROM Course_Data WHERE Course_No LIKE \'%" + course + "%\'")
    data = cur.fetchall()
    strg = "  "
    i=0
    if len(data) == 0:
        lab.config(text="    No courses found!")
    else:
        for row in data:
            i+=1
            strg += (row[0]) + " | "
            if i%8==0:
                strg+="\n\n  "
        lab.config(text=strg)

def clearDisplay(root, conn, lab):
    lab.config(text="")
...
def makeform(root, fields, base):
    ...
    lr = []
    lc = []
    table = Frame(root).grid(row=0, column=12 , rowspan=21, columnspan=10, sticky = N)
    for i in range(0,20):
        s = "row" + str(i+1)
        lr.append(Label(table, text = s));
    for i in range(1,9):
        s = "Slot" + chr(ord('A')+int(i-1))
        lc.append(Label(table, text = s));
    for i in range(0,20):
    ...
    for i in range(0,160):
        e.append(Label(table, text = "-", bg = colours[int(i/20)],borderwidth=2, relief="groove", width=9, height=2));
...
def searchbox(root):
    entries = {}
    row1 = Frame(root).grid(row = 1, column = 26, sticky = W, padx = 3, pady = 1, rowspan=1, columnspan=8)
    lab1 = Label(row1, text="      Search for courses using its initials, etc", anchor='w', fg="green", font=(None, 15)).grid(row = 1, column = 26, sticky = W, padx = 3, pady = 1, rowspan=1, columnspan=8);
    row2 = Frame(root)
    lab2 = Label(row2, width=10, text="       Search: ", anchor='w')
    ent = Entry(row2)
    # ent.insert(0,"0")
    row2.grid(row = 2, column = 27, sticky = W, padx = 3, pady = 1)
    lab2.grid(row = 2, column = 27, sticky = W, padx = 1, pady = 1)
    ent.grid(row = 2, column = 28, sticky = W, padx = 1, pady = 1)
    entries["Search"] = ent

    return entries

def Show_leftouts(root, conni, entries):
    cur = conni.cursor()
    cur.execute("SELECT * FROM Course_Data A WHERE NOT EXISTS(SELECT Course_No FROM Allocated_Subjs WHERE Course_No = A.Course_No)")
    data = cur.fetchall()
    for row in data:
        print(row[1])
    conni.commit()
...
if __name__ == '__main__':
   ...
   ents3 = searchbox(root)
   row = Frame(root).grid(row=5, column=26, columnspan=8, rowspan=15)
   lab = Label(row, width=60 ,text="", anchor='w')
   lab.grid(row=5, column=26, columnspan=8, rowspan=15)
   butt_row3 = Frame(root)
   butt_row3.grid(row = 3, column = 27, sticky = W, padx = 1, pady = 1, columnspan=8)
   b13 = Button(butt_row3, text = 'Search Database', command=(lambda e = ents3: searchCourse(root,e, conn, lab))).grid(row = 3, column = 27,sticky = W, padx = 1, pady = 1)
#    b15 = Button(butt_row3, text = '  Search Timetable', command=(lambda e = ents3: searchCourseT(root,e, conn, lab))).grid(row = 3, column = 27,sticky = W, padx = 1, pady = 1)
   b14 = Button(butt_row3, text = 'Clear', command=(lambda e = ents3: clearDisplay(root, conn, lab))).grid(row = 3, column = 28,sticky = W, padx = 1, pady = 1)
   b15 = Button(root, text = 'Display Courses Left', command=(lambda e = ents: Show_leftouts(root, conn, e))).grid(row = 5, column = 27,sticky = W, padx = 1, pady = 1)
   root.mainloop()
   if conn:
       conn.close()
